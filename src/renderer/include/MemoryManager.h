...
*/
class _RendererExport MemoryManager {
...
inline void *operator new(size_t reportedSize) {
  if( !gProcessID )
    gProcessID = renderer::MemoryManager::sMemManager._getProcessID();
  return renderer::MemoryManager::sMemManager.op_new_sc( reportedSize, gProcessID );
}
inline void *operator new[](size_t reportedSize) {
  if( !gProcessID )
    gProcessID = renderer::MemoryManager::sMemManager._getProcessID();
  return renderer::MemoryManager::sMemManager.op_new_vc( reportedSize, gProcessID );
}

inline void operator delete(void *reportedAddress) {
  renderer::MemoryManager::sMemManager.op_del_sc( reportedAddress, gProcessID );
}
inline void operator delete[](void *reportedAddress) {
  renderer::MemoryManager::sMemManager.op_del_vc( reportedAddress, gProcessID );
}
//-----------------------------------------------------------------------------
...
inline void *operator new( size_t count ) {
  return renderer::MemoryManager::sMemManager.allocMem( "", 0, count );
}
...
inline void operator delete( void* ptr ) {
  renderer::MemoryManager::sMemManager.dllocMem( "", 0, ptr );
}
