...
Win32ConfigDialog::Win32ConfigDialog(HINSTANCE app_instance) {
  app_instance_ = app_instance;
  selected_render_system_ = 0;
}
...
BOOL Win32ConfigDialog::DlgProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INITDIALOG:
    ...
    dlg->selected_render_system_ = Root::getSingleton().getRenderSystem();
    ...
    while (pRend != lstRend->end()) {
      ...
      if (*pRend == dlg->selected_render_system_) {
        // Select
  ...
  case WM_COMMAND:
    ...
    case IDC_CBO_RENDERSYSTEM:
      ...
      if (HIWORD(wParam) == CBN_SELCHANGE ) {
        ...
        if (sel != -1) {
          ...
          pRend = lstRend->begin();
          dlg->selected_render_system_ = pRend[sel];
          // refresh options
          // Get options from render system
    ...
    case IDC_CBO_OPTION:
      if (HIWORD(wParam) == CBN_SELCHANGE) {
        ...
        if (sel != -1) {
          ...
          // Set option
          dlg->selected_render_system_->setConfigOption(
            pOpt->second.name, pPoss[sel]);
          // Re-retrieve options
          opts = dlg->selected_render_system_->getConfigOptions();
    ...
    case IDOK:
      // Set render system
      if (!dlg->selected_render_system_) {
        MessageBox(NULL, "Please choose a rendering system.", "OGRE", MB_OK | MB_ICONEXCLAMATION);
        return TRUE;
      }
      err = dlg->selected_render_system_->validateConfigOptions();
      if (err.length() > 0) {
      ...
      Root::getSingleton().setRenderSystem(dlg->selected_render_system_);
...
bool Win32ConfigDialog::Display(void) {
  ...
  int i;
  dlg = this;
  i = DialogBox(app_instance_, MAKEINTRESOURCE(IDD_DLG_CONFIG), NULL, DlgProc);
